...
#        self.assertRegex(output, 'tests/test_harness.allocation_test.*OK')
    ...
    def testTopLevelMinParallel(self):
        """
        Scenario which tests top-level floor
        """
        # Passing test triggering no caveats, as supplied allocations satisfies
        # the test's requirements
        output = self.runTests('-i', 'parallel_top_block', '--no-color').decode('utf-8')
        print(output)
        self.assertRegex(output, r'tests/test_harness.override_of_two.*?MIN_CPUS=2,OVERSIZED.*?OK')
